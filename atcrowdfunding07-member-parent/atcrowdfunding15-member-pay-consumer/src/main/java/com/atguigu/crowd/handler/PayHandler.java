	...
	*/
    ...
    public String returnUrlMethod(HttpServletRequest request, HttpSession session) throws UnsupportedEncodingException, AlipayApiException {
        ...
        if(signVerified) {
            ...
            return "trade_no:"+orderNum+"<br/>out_trade_no:"+payOrderNum+"<br/>total_amount:"+totalAmount;
        }else {
            return "验签失败";
        }
    }

    //这里加ResponseBody让当前的返回值成为响应体，在浏览器上显示支付宝支付界面
    @ResponseBody
    @RequestMapping("/generate/order")
    public String generateOrder(HttpSession session, OrderVO orderVO) throws AlipayApiException {

        logger.info("addressId=" + orderVO.getAddressId());
    ...
    /**
     * 为了调用支付宝接口封装的方法
     * @param outTradeNum 外部订单号，商户订单号，也就是我们生成的订单号
     * @param totalAmount 订单的总金额
     * @param subject 订单的标题，使用项目名称
     * @param body 商品描述，使用回报描述
     * @return 返回到页面显示支付宝登陆界面
